...
> 最后更新：2026-09-19　版本：v1.6.1
> 本地占用：**约 10.5 MB**（详见第 15.1 节空间账本）；日常维护看第 15 节。
...
### v1.6.1（2026-09-19）本地瘦身 + 维护手册

麦冬问「本地需要多少空间、是不是都上传了、给我删掉无需的东西」+ 要求写维护教程。

**实测空间账本**（不是估算）：
| 项 | 删前 | 删后 |
|---|---|---|
| 项目本体（非 .git，20 个文件） | 11.72 MB | **7.68 MB** |
| `.git` | 2.82 MB | 2.82 MB（保留） |
| `data/questions/raw/` | 4.04 MB | **0（删）** |
| 合计 | 14.54 MB | **10.50 MB** |

**删了什么**：`data/questions/raw/` 38 个文件 4.04 MB —— `fetch_tcmle.py` 下载题库的落地缓存，
被 `.gitignore` 排除、从未入库，`questions.json` 已是成品。删除后已验证项目完整（关键文件体检全绿）。

**没删什么及为何**：
- `data/lectures/raw/haowanshan_raw.md`（2.01 MB）—— 幕布导出的讲稿**原件**，幕布链接可能失效，
  这是整条讲稿链路的根，GitHub 那份也是它的拷贝而非第二来源
- `.git`（2.82 MB）—— 后续还需 push

**新增第 15 节「本地空间账本与日常维护手册」**：空间账本、可再生/不可再生分类、
「删了 raw 就别再跑 fetch_tcmle.py」的铁律、三处数据存放对照、标准改动五步、
常见改动速查表、故障速查表、每月/每季清单、彻底弃坑流程。

**同步更新**：
- `README.md` 全面重写（旧版说"6 位角色/五个页面"，实际已是 7 位角色 / 七个页面；补全文件清单）
- `使用说明.md` 新增第十节「以后怎么维护（不用会写代码）」
- 第 13.1 节补「已清理项」；13.4 清单打勾

**排查到的小坑**：`git ls-files` / `git status` 默认 `core.quotepath=true`，
**中文文件名会输出成 `"\344\275\277..."` 八进制转义**，自动化脚本据此判断"是否入库"会误判
（本次就一度误以为 `使用说明.md` 没入库）。查中文文件名用 `git -c core.quotepath=false ls-files`。
...
**study-ai 本体内已于 2026-09-19 清理的**：

| 文件/目录 | 体积 | 处置 | 理由 |
|---|---|---|---|
| `data/questions/raw/`（38 个 JSON） | **4.04MB** | 🗑 **已删** | 题库下载缓存，被 `.gitignore` 排除、从未入库，`questions.json` 已是成品。<br>⚠️ **删后别再跑 `fetch_tcmle.py`** —— 会重排题号导致答题记录串号，详见第 15.3 节 |
...
- [ ] 导出一次 JSON 备份存档
- [x] 删 `study-ai/data/questions/raw/`（**2026-09-19 已删**，释放 4.04 MB）
...
## 15. 本地空间账本与日常维护手册（v1.6.0 实测）

> 本节回答三个问题：**本地到底占多少、哪些能删、以后怎么维护**。
> 数据均为 2026-09-19 实测，不是估算。

### 15.1 空间账本

| 项 | 删前 | 删后（当前） | 说明 |
|---|---|---|---|
| 项目本体（非 `.git`） | 11.72 MB | **7.68 MB** | 真正干活的文件，20 个 |
| `.git`（本地仓库历史） | 2.82 MB | **2.82 MB** | 保留，否则没法推代码 |
| `data/questions/raw/` | 4.04 MB | **0（已删）** | 题库下载缓存，可再生 |
| **合计** | **14.54 MB** | **10.50 MB** | 就这些，不用担心空间 |

**结论：本地只需要 10.5 MB。** 这不是"省着点用"的量级，正常保留即可，不用刻意瘦身。

体积构成看一眼就明白：

| 文件 | 大小 | 性质 |
|---|---|---|
| `data/questions/questions.json` | 3.02 MB | 题库成品（7284 题），已入库 |
| `data/lectures/haowanshan.json` | 2.13 MB | 讲稿成品（2256 段），已入库 |
| `data/lectures/raw/haowanshan_raw.md` | 2.01 MB | **讲稿原件，见 15.2** |
| `outline.json` | 0.32 MB | 大纲 1025 细目 |
| 其余 16 个（代码/函数/文档/工具） | 0.2 MB | 都很小 |

### 15.2 三类东西，删之前先分清

**① 可再生（删了还能再来，放心删）**
- `data/questions/raw/` —— `fetch_tcmle.py` 下载题库时的落地缓存。**已于 2026-09-19 删除**，释放 4.04 MB
- `node_modules/` —— 本项目没有（依赖只有 `@netlify/blobs`，由 Netlify 云端安装）

**② 不可再生（删了真没了，别删）**
- `data/lectures/raw/haowanshan_raw.md` —— **你从幕布导出的《伤寒论讲稿》原件**。
  `haowanshan.json` 是它加工出来的，幕布分享链接 `share.mubu.com/doc/3CCri42ou-B` 随时可能失效。
  这 2 MB 是整条讲稿链路的根，删了就只能去找幕布要。
  （GitHub 上有备份，但那也是同一份，不是第二来源）
- **Netlify Blobs 里的运行时数据** —— 打卡、大纲进度、聊天记录、答题记录、角色私设。
  这些**不在你的硬盘上**，在云端。清浏览器缓存不会掉，但删 Netlify 站点会真的没。

**③ 中间层（本地删了能从 GitHub 拉回来）**
- `questions.json` / `haowanshan.json` / `outline.json` / 所有代码和文档 —— 全部已入库。
  真删了：`git clone git@github.com:yidaoyi/study-ai.git` 就回来了。
- 已全部验证过：git 追踪 **20 个文件**，一个不少（中文文件名在 `git status` 里会显示成
  `"\344\275\277..."` 这种八进制转义，**不是没入库**，用 `git -c core.quotepath=false ls-files` 看才正常）

### 15.3 ⚠️ 删掉 raw 之后的铁律：别再跑 fetch_tcmle.py

`tools/fetch_tcmle.py` 重跑会重新下载 38 个原始 JSON 并重新生成 `questions.json`
→ **题号大概率重排** → 你之前所有答题记录按题号索引，**全部串号**（见第 9 节坑⑩）。

| 想做什么 | 该怎么做 |
|---|---|
| 只是想删本地缓存省空间 | 已删完，**到此为止，什么都别再动** |
| 想更新题库（加新题） | 先设置页导出 JSON 备份；接受"答题记录可能串号"；再跑脚本 |
| 想改某一道题的内容 | **直接改 `questions.json`**，不要重跑脚本 |

### 15.4 你的数据到底存在哪（三份，别搞混）

| 位置 | 存了什么 | 丢了怎么办 |
|---|---|---|
| **GitHub**（`yidaoyi/study-ai`） | 代码、题库、讲稿、文档 —— 项目的"本体" | 基本上丢不了 |
| **Netlify Blobs**（云端，美国） | 你的**个人数据**：打卡 / 大纲进度 / 聊天 / 答题 / 私设，按身份码分片 | **这是唯一真源**，本站没有任何本地副本。每月导出一次 JSON 兜底 |
| **浏览器 localStorage** | 上面那份的离线缓存 | 清缓存会丢 —— 但**只是缓存**，重新联网会从云端拉回来 |

⚠️ 反过来说：**清浏览器缓存不会丢进度**，因为真源在云端。但**删 Netlify 站点会真丢** —— 删之前一定先导出。

### 15.5 标准改动五步（每次改代码都这么走）

| # | 步骤 | 谁 | 耗时 |
|---|---|---|---|
| 1 | 改文件 | 你/AI | — |
| 2 | 语法体检：`node --check app.js`（改哪个查哪个） | — | 5 秒 |
| 3 | `git add -A` → `git commit -m "说明"` | — | 10 秒 |
| 4 | `git push origin HEAD:main` | — | 20 秒 |
| 5 | 打开 Netlify 后台看 deploys 变绿，再去线上站点一遍 | 你 | 2 分钟 |

- 本项目**无构建步骤** —— push 完 Netlify 自动部署，通常 1 分钟内
- 本地预览：`双击启动.bat`（起一个静态服务器）
  ⚠️ **不要双击 `index.html`**，`file://` 协议读不到 `outline.json` 和题库，页面会是空的
- 本地预览**用不了 AI**（`/.netlify/functions/*` 只在 Netlify 上存在），这是正常的

### 15.6 常见改动去哪改（速查表）

| 想改什么 | 改哪个文件 | 要不要重新部署 |
|---|---|---|
| 角色官方人设 / 台词 / 防脱题 | `netlify/functions/crew-data.js` | 要 |
| 前端角色卡片（名字/emoji/标签） | `app.js` 的 `CREW_UI` | 要 |
| **角色私设**（自己的设定） | **不用改代码** —— 设置页「角色私设」里填 | 不用 |
| 加新 AI 角色 | `crew-data.js` 加条目 + `app.js` 的 `CREW_UI` 同步 | 要 |
| AI 模型 / 温度 | `chat.js` 的 `MODEL`、`temperature` | 要 |
| 访客额度（400/天、40/人） | `chat.js` 的 `DAILY_GUEST_TOTAL`、`DAILY_GUEST_LIMIT` | 要 |
| 每日提醒时间（7点/13点） | `netlify.toml` 的 schedule | 要 |
| 提醒文案风格 | `remind.js` 的 `generateReminder()` | 要 |
| 界面样式 / 主题配色 | `index.html` 的 CSS 变量 `--bg/--fg/--card/--line/--muted` | 要 |
| 打卡音效 | `app.js` 里搜 `sfx`（Web Audio 合成） | 要 |
| 大纲内容 | `outline.json` | 要 |
| 某道题的内容 | `data/questions/questions.json`（**直接改，别重跑脚本**） | 要 |
| 伤寒判定词表 | `app.js` 的 `SHANG_STRONG` | 要 |
| 讲稿内容 | 改 `data/lectures/raw/haowanshan_raw.md` 后跑 `tools/build_lecture.py` 重新生成 json | 要 |
| 文档 | `README.md` / `使用说明.md` / 本文件 | 不用（不影响站点） |

**规则（第 11 节已有，这里重申）**：改完顺手在第 12 节追加一条版本记录，写清楚改了什么、为什么。

### 15.7 故障速查

| 现象 | 原因 / 怎么办 |
|---|---|
| 页面空白、一直"载入中" | ① 是不是双击了 `index.html`（应用 `双击启动.bat`）；② Netlify 部署失败（后台看 deploys 日志） |
| AI 不回话 | 本地预览是正常的；线上不回 → Netlify 后台确认 `ZHIPU_API_KEY` 还在、没欠费 |
| 提示"今天陪人陪累了" | 全站访客额度用完了（`DAILY_GUEST_TOTAL`）。你自己不受影响（主人不限额）；要放宽就改那个数再部署 |
| 打卡/进度突然空了 | 大概率是**换了身份码或用了不带 `?u=` 的网址**。打开你的专属链接就回来 |
| 答题记录全部串号 | 题库被重排过，见 15.3 和第 9 节坑⑩ |
| 大纲勾选互串 | 老 bug，已用层级路径 key 修掉，不会再犯（坑①） |
| 企业微信收不到提醒 | `WECOM_WEBHOOK_URL` 没配或机器人被移出群；到 Netlify 后台 Function 日志看报错 |
| `Permission denied (publickey)` | SSH 问题，见第 9 节坑⑧（本机配了 `-i` 指定私钥） |
| 想彻底推倒重来 | 见 15.9 |

### 15.8 定期清单

**每月一次（3 分钟）**
- [ ] 设置页 → 导出备份 JSON → 丢网盘/QQ收藏（这是云数据的兜底）
- [ ] 看一眼 Netlify 后台的 Functions 调用量，确认没被人刷

**每季度 / 每次大改之前**
- [ ] `git log --oneline -10` 看一眼最近改了什么
- [ ] 确认 Netlify Blobs 还在（删站点 = 删数据）

**换电脑 / 重装系统后**
- [ ] `git clone git@github.com:yidaoyi/study-ai.git`
- [ ] 配 SSH：本机已固化私钥路径，见第 9 节坑⑧
- [ ] 打开你的专属链接 `?u=maidong`，数据自动从云端回来

### 15.9 想彻底不要了，怎么收干净

按顺序：
1. **导出 JSON 备份**（设置页）← 这一步绝不能跳
2. Netlify：删站点（这会同时删掉 Blobs 里的所有个人数据）
3. GitHub：仓库 Settings → 转 Private 或 Delete
4. 本地：删 `study-ai` 文件夹（10.5 MB）
5. 顺手清理：智谱 key（如果不再用）、企业微信群机器人

**只是想让别人也能看源码但看不到数据**：数据本来就不在仓库里，保持现状即可。
